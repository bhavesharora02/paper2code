...
Usage:
  1) Activate venv
  2) pip install Jinja2 pyyaml
  3) python tools/bootstrap_week5_safe.py   # creates safe templates + generator + CLI + test
...
This is the "safe" variant of the bootstrap that writes smoke-friendly templates by default.
"""
...
def main():
    ...
    write_file(gen_pkg_init, "# package: core.codegen\n", args.force)

    gen_mod = ROOT / 'core/codegen/generate_repo.py'
    # Minimal generate_repo implementation (keeps previous behavior)
    gen_code = r"""from __future__ import annotations
import json
from pathlib import Path
...
def generate_repo(ir_path: Path, mapping_path: Path, out_dir: Path, templates_dir: Path) -> None:
    ...
    for name in ("model.py.jinja", "trainer.py.jinja", "data_loader.py.jinja", "config.yaml.jinja", "README.jinja"):
        ...
        target_name = name.replace('.jinja', '')
...
"""
    write_file(gen_mod, gen_code, args.force)

    # CLI app
    app = ROOT / 'apps/cli/generate_all.py'
    cli_code = r"""from __future__ import annotations
import argparse
import json
from pathlib import Path
...
if __name__ == '__main__':
    main()
"""
    write_file(app, cli_code, args.force)
...
"""
    write_file(test, test_code, args.force)

    if wrote:
        print('[bootstrap_week5_safe] wrote:')
        for p in wrote:
            print('  -', p.relative_to(ROOT))
    print('[bootstrap_week5_safe] ensured core/codegen and CLI + test files (safe templates)')
